...
echo "=== 02-dotfiles-sway.sh: Writing configuration files ==="
...
### Appearance (Adwaita/GNOME palette)
...
### Wallpaper (ships with the gnome-backgrounds package, nothing to download)
output * bg /usr/share/backgrounds/gnome/adwaita-d.jxl fill

### Per-device keyboards
### IMPORTANT: verify the real identifiers on YOUR machine with:
###   swaymsg -t get_inputs | jq -r '.[] | "\(.identifier) — \(.name)"'
### The ones below matched the previous install — they may differ on
### different hardware.
input "type:keyboard" {
    xkb_layout us
}
...
### Monitor assignment
### Workspaces 1-4: external monitor (falls back automatically to the
### laptop screen if it isn't connected). Workspace 5: dedicated to the
### laptop, so Sway doesn't "steal" a random number when booting without
...
### Everyday app layout
### IMPORTANT: the app_ids below were confirmed on the previous install.
### Verify yours with the app open:
...
### Small apps: floating, fixed size, centered
...
### Authentication agent (polkit-gnome is discontinued on Fedora, use
### mate-polkit instead). VERIFY the real path with:
...
# ============================================================
# GAMMASTEP (Night Light — geoclue2 timed out last time, use manual coords)
# ============================================================
...
echo "=== 02-dotfiles-sway.sh completed ==="
echo ">>> BEFORE logging into Sway, verify:"
echo ">>> 1. rpm -ql mate-polkit | grep bin   (confirm the executable path)"
echo ">>> 2. Keyboard identifiers (swaymsg -t get_inputs) after the first login"
echo ">>> 3. Chrome/Firefox/Nautilus app_ids after opening each once"
